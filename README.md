### 2025-12-15 10:36:38
TODO: 手动报告界面 + 初始化引导 + debug + 引导文字
1. 将进度和用户填写的信息放到额外的单独页面一起显示，现在是弹窗，且关闭后就没了，固定在单一页面内
2. 用户初始化过程优化
3. api 优化 ，为什么会重复调用那么多次，逻辑debug（不重要，可以后面慢慢优化
4. 测试多用户报告生成功能
5. 增加用户笔记功能
6. 首页显示：已为您生成个性化论文/报告 XX 篇，今天是为您服务的第 xx 天
7. 第一次使用的用户在周末生成报告的话，显示的不应该是当日的日期，而是论文实际挂在arxiv上的日期
